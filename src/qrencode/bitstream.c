...
//将数字num编码为长度为bits字节的bit流，每一个字节对应着num的二进制的每一位
...
//在bstream后面，附加bits长度位，用于表示数字num的bit流
