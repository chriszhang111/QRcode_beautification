...
typedef struct {
	int dataLength;     //数据codewords
	unsigned char *data;//数据内容
	int eccLength;      //校正码codewords
	unsigned char *ecc; //校正码内容
} RSblock;

typedef struct {
	int version;            //版本
	int dataLength;         //数据的总codewords
	int eccLength;          //校验码的总codewords
	unsigned char *datacode;//数据
	unsigned char *ecccode; //校验码
	int b1;                 //第一种类型的块数
	int blocks;             //总块数
	RSblock *rsblock;       //RSblock序列,共blocks块
	int count;
} QRRawCode;
...
__STATIC QRRawCode *QRraw_new(QRinput *input)
{
	...
	raw->version = input->version;              //版本
	raw->b1 = QRspec_rsBlockNum1(spec);         //第一种类型的块数
	raw->dataLength = QRspec_rsDataLength(spec);//数据的总codewords
	raw->eccLength = QRspec_rsEccLength(spec);  //校验码的总codewords
    raw->ecccode = (unsigned char *)malloc(raw->eccLength);//校验码
	if(raw->ecccode == NULL) {
		free(raw->datacode);
		free(raw);
		return NULL;
	...
	raw->blocks = QRspec_rsBlockNum(spec);      //总块数
	raw->rsblock = (RSblock *)calloc(raw->blocks, sizeof(RSblock));//按照块数，分配并初始化RSblock
	if(raw->rsblock == NULL) {
		QRraw_free(raw);
		return NULL;
	}
...
__STATIC QRcode *QRcode_encodeMask(QRinput *input, int mask)
{
	...
	for(i=0; i<raw->dataLength + raw->eccLength; i++) {
		...
		bit = 0x80;
		for(j=0; j<8; j++) {
			p = FrameFiller_next(filler);    //获取下一个需要填充的位置地址
			if(p == NULL)  goto EXIT;
			*p = 0x02 | ((bit & code) != 0); //与0x02或运算，将第2位置为1，代表该为数据或校验区域
			bit = bit >> 1;
		}
	}
	QRraw_free(raw);
